...
import { BookingRepository, SeatRepository } from './bookingRepository';
...
export interface BookingService {
  ...
  findByUserId(userId: string, params?: { page?: number; limit?: number }): Promise<PaginatedResponse<any>>;
...
export class BookingServiceImpl implements BookingService {
  ...
  async findByUserId(userId: string, params: { page?: number; limit?: number } = {}): Promise<PaginatedResponse<any>> {
    const page = Number(params.page) || 1;
    const limit = Number(params.limit) || 10;
    const offset = (page - 1) * limit;

    const [items, total] = await Promise.all([
      this.bookingRepository.findByUserIdWithDetails(userId, limit, offset),
      this.bookingRepository.countByUserId(userId)
    ]);
